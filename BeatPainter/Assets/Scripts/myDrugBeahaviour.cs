...
public class myDrugBeahaviour : MonoBehaviour
{
    ...
    void Start()
    {
Debug.Log("Camera. The camera name is: " + Camera.main.name);
    }
...
Debug.Log("Camera. Touching at: " + touch.position);

            //Gets the ray at position where the screen is touched
            Ray ray = Camera.main.ScreenPointToRay(touch.position);

Debug.Log("Camera. Ray is " + ray);

            if (touch.phase == TouchPhase.Began)
            {
                Debug.Log("Camer. Touch phase began at: " + touch.position);
