...
namespace CustomHighPass
{
	...
	{	//Called for every plugin instance
		...
		vpData->mpHPFFilter = new BiquadFilter();
		vpData->mpHPFFilter->SetupHighpass(5000, state->samplerate, 0.5);
	...
	{	//Simple parameter callback, gets the value from gui and sets the parameter
		...
		{
			vpData->mpHPFFilter->SetupHighpass(vpData->mParams[P_CUTOFF], state->samplerate, vpData->mParams[P_Q]);
		}
		return UNITY_AUDIODSP_OK;
	}
	...
	UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK ProcessCallback(UnityAudioEffectState* state, float* inbuffer, float* outbuffer, unsigned int length, int inchannels, int outchannels)
	{
		...
		for (unsigned int n = 0; n < length; n++)
		{
			...
			{	//Interleaved buffer parsing
				...
				{
					outbuffer[n * outchannels + i] = vpHPF->Process(inbuffer[n * outchannels + i]);
				}
				else
				{
